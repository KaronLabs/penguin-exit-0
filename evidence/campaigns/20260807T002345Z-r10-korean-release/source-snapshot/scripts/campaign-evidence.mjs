export function parsePassingBrowserCounts(text, exitCode) {
    ...
    const exactFinalSummaries = normalizedLines.filter((line) => /^\s*39\s+passed\s+\([^)]+\)\s*$/i.test(line));
    ...
    for (const line of normalizedLines) {
        ...
        if (denominator !== 39 || index < 1 || index > 39 || !engines.includes(engine) || seenIndices.has(index)) {
    ...
    const expectedIndicesPresent = seenIndices.size === 39;
    const engineCountsAreExact = engines.every((name) => counts[name].passed === 13);
...
export function browserCountsAreComplete(counts) {
    return counts.integrity === true && ['chromium', 'firefox', 'webkit'].every((name) => counts[name].passed === 13 && counts[name].failed === 0);
}
